...
class Patcher implements Plugin<Project>{
    ...
    void apply(Project project) {
        ...
        if (!isTransformAvailable()) {
            throw new GradleException('Patcher gradle plugin only supports android gradle plugin 2.0.0 or later')
        }

        // バイトコード変換を行うTransformerを登録する
        project.android.registerTransform(new PatcherTransformer(project))
    }
